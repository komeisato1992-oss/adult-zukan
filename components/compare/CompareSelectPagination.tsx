...
import type { SimilaritySort } from "@/lib/compare/similarity";

type CompareSelectPaginationProps = {
  contentId: string;
  currentPage: number;
  totalPages: number;
  sort: SimilaritySort;
...
export function CompareSelectPagination({
  contentId,
  currentPage,
  totalPages,
  sort,
}: CompareSelectPaginationProps) {
  if (totalPages <= 1) return null;

  const basePath = `/compare/select/${contentId}`;
  const query = {
  ...
  function trackPage(page: number) {
    trackCompareEvent(COMPARE_GA_EVENTS.pageChange, {
