...
export type CompareCandidateGuidePayload = {
  contentId: string;
  title: string;
};

const GUIDE_EVENT = "az:compare-candidate-guide";

export function openCompareCandidateGuide(
  payload: CompareCandidateGuidePayload,
): void {
...
export function CompareCandidateGuide() {
  const titleId = useId();
  const [payload, setPayload] = useState<CompareCandidateGuidePayload | null>(
    null,
  );

  useEffect(() => {
    const onGuide = (event: Event) => {
      const detail = (event as CustomEvent<CompareCandidateGuidePayload>).detail;
      if (!detail?.contentId) return;
      setPayload(detail);
      trackCompareEvent(COMPARE_GA_EVENTS.candidatePopupShow, {
  ...
  const similarHref = buildCompareSelectHref(payload.contentId);
      ...
      >
        ...
        <div className="mt-4 space-y-2">
          <Link
            href={similarHref}
            onClick={() => {
              trackCompareEvent(COMPARE_GA_EVENTS.seeSimilarClick, {
          ...
          <Link
            href="/works"
            onClick={() => {
              trackCompareEvent(COMPARE_GA_EVENTS.browseListClick, {
                content_id: payload.contentId,
              });
              close();
            }}
          ...
          <button
            type="button"
            onClick={() => {
              trackCompareEvent(COMPARE_GA_EVENTS.laterClick, {
                content_id: payload.contentId,
              });
              close();
            }}
