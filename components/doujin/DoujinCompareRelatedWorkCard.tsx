...
type DoujinCompareRelatedWorkCardProps = {
  work: DoujinSimilarWorkCardData;
  source: "both" | "a" | "b" | "multi";
};

export function DoujinCompareRelatedWorkCard({
  work,
  source,
}: DoujinCompareRelatedWorkCardProps) {
  const router = useRouter();
  const [inCompare, setInCompare] = useState(() => isDoujinCompareId(work.workId));
  const [count, setCount] = useState(() => readDoujinCompareIds().length);

  useEffect(() => {
    const sync = () => {
      setInCompare(isDoujinCompareId(work.workId));
      setCount(readDoujinCompareIds().length);
    };
    sync();
    return subscribeDoujinCompareStore(sync);
  }, [work.workId]);

  const atLimit = count >= DOUJIN_COMPARE_MAX_ITEMS && !inCompare;
  const disabled = inCompare || atLimit;
  const label = inCompare ? "比較中" : atLimit ? "比較上限" : "比較＋";
  const reasons = work.reasonLabels.slice(0, 2);

  function handleAdd() {
    if (disabled) return;

    const result = addDoujinCompareId(work.workId);
  ...
  return (
    <article className="group flex h-full min-w-0 flex-row gap-3 overflow-hidden rounded-lg border border-border/80 bg-white p-3 shadow-sm transition-all hover:-translate-y-0.5 hover:border-accent/20 hover:shadow-md max-[768px]:items-stretch min-[769px]:flex-col min-[769px]:gap-0 min-[769px]:p-0">
      <Link
        ...
        onClick={() =>
          trackDoujinCompareEvent(DOUJIN_COMPARE_GA_EVENTS.relatedClick, {
            content_id: work.workId,
            source,
            similarity: work.similarityScore,
          })
      ...
      <div className="flex min-w-0 flex-1 flex-col max-[768px]:w-[65%] min-[769px]:px-2.5 min-[769px]:pb-2.5 min-[769px]:pt-2.5">
        <Link href={`/doujin/works/${work.workId}`} prefetch className="min-w-0">
        ...
        {work.circleName || work.authorNames.length > 0 ? (
          <p className="mt-1 line-clamp-2 text-xs text-muted">
            {[work.circleName, work.authorNames.slice(0, 2).join("、")]
              .filter(Boolean)
              .join(" / ")}
          </p>
        ) : null}
        {work.price != null ? (
          <p className="mt-1 text-sm font-bold text-price">
            {formatDoujinComparePriceYen(work.price)}
          </p>
        ) : null}
        <div className="mt-1 rounded-md bg-accent-light px-2 py-1.5 max-[768px]:bg-transparent max-[768px]:px-0 max-[768px]:py-0">
        ...
        <div className="mt-auto grid grid-cols-2 gap-1.5 pt-2.5 max-[768px]:gap-2">
          <button
            ...
            className={`${workCardCtaBaseClassName} min-h-10 border max-[768px]:min-h-11 ${
              disabled
                ? "cursor-not-allowed border-border bg-accent-light text-accent opacity-80"
